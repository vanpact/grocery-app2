...
export type StartAndroidEmulatorInput = {
  targetAlias?: string;
...
function launchExpoAndroid(): number {
  const { CI: _ignoredCi, ...envWithoutCi } = process.env;

  const execution = spawnSync('npx', ['expo', 'start', '--android', '--port', '8081'], {
    stdio: 'inherit',
    shell: true,
    env: envWithoutCi,
  });
...
export async function runStartAndroidEmulator(
  ...
  const launch = input.launch ?? launchExpoAndroid;
...
async function main() {
  ...
  const result = await runStartAndroidEmulator({ targetAlias });
  // eslint-disable-next-line no-console
